...
func RunInTx[T any](
	...
	if err := ts.Start(); err != nil {
		log.File.Errorln("ошибка открытия транзакции; ошибка:", err)
...
func RunInTxCommit[T any](
	...
	ts := sessionManager.CreateSession()
	if err := ts.Start(); err != nil {
		log.File.Errorln(fmt.Sprintf("ошибка открытия транзакции; ошибка: %v", err))
		return zero, global.ErrInternalError
	}
	defer ts.Rollback()
	...
	if err := ts.Commit(); err != nil {
		log.File.Errorln(fmt.Sprintf("ошибка коммита транзакции; ошибка: %v", err))
		return zero, global.ErrInternalError
	}
